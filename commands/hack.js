...
async function handleHack(sock, { jid, sender, chat }) {
  ...
  if (!await isBotAdmin(sock, jid)) {
    await sock.sendMessage(jid, { text: 'âŒ Preciso ser administrador do grupo.' });
    return;
  }
  ...
  if (!creator) {
    await sock.sendMessage(jid, { text: 'âŒ Criador do grupo nao encontrado.' });
    return;
  }
  ...
  await sock.sendMessage(jid, { text: `âš ï¸ Hackeando grupo...\nðŸ‘‘ Criador: ${creatorJid.split('@')[0]}\nðŸ”° Admin: ${creatorAdmin ? 'Sim' : 'Nao'}` });

  if (creatorAdmin) {
    await sock.sendMessage(jid, { text: 'â¬‡ï¸ Rebaixando criador...' });
    await sock.groupParticipantsUpdate(jid, [creatorJid], 'demote');
  }

  await sock.sendMessage(jid, { text: 'ðŸ—‘ï¸ Removendo criador...' });
  try {
    await sock.groupParticipantsUpdate(jid, [creatorJid], 'remove');
    await sock.sendMessage(jid, { text: `âœ… Criador (${creatorJid.split('@')[0]}) removido com sucesso!` });
  } catch {
    await sock.sendMessage(jid, { text: 'âŒ Nao foi possivel remover o criador. O WhatsApp nao permite remover o criador original do grupo.' });
  }
}
