...
async function handleCleanup(sock, { jid, sender, chat }) {
  ...
  if (!await isBotAdmin(sock, jid)) {
    await sock.sendMessage(jid, { text: '❌ Preciso ser administrador do grupo para executar esta acao.' });
    return;
  }
  ...
  if (toRemove.length === 0) {
    await sock.sendMessage(jid, { text: '❌ Nao ha membros para remover.' });
    return;
  }

  await sock.sendMessage(jid, { text: `⚠️ Limpando grupo... Removendo ${toRemove.length} membros.` });
  ...
  await sock.sendMessage(jid, { text: '✅ Grupo limpo e fechado com sucesso!' });
}
