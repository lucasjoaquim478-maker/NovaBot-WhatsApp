...
class Updater extends EventEmitter {
  ...
  async _autoCheckAndUpdate() {
    try {
      ...
      if (result && result.hasUpdate && !this._autoUpdateRunning) {
        this._autoUpdateRunning = true;
        this._log('info', `Auto-update detectou v${result.version}, baixando...`);
        await this.performUpdate(result.version);
        this._log('info', 'Auto-update concluído, reiniciando...');
        setTimeout(() => safeRestart(), 2000);
      }
    } catch (err) {
      this._log('warn', `Auto-update falhou: ${err.message}`);
    }
    this._autoUpdateRunning = false;
  }
