...
async function downloadFile(repo, filePath, branch) {
  ...
  if (res.status === 404) { logger.debug(`[UPDATE] ${filePath} não encontrado no repositório, pulando.`); return null; }
  ...
  try {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        if (content === null) continue; // 404 - arquivo não existe no repositório
    ...
    if (hasPackageChange) {
      ...
      try {
        ...
        const npmCmd = process.platform === 'win32'
          ? path.join(process.cwd(), 'node', 'npm.cmd')
          : path.join(process.cwd(), 'node', 'npm');
