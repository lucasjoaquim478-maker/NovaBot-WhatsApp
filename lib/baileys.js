...
async function doConnect() {
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (qr) {
      ...
      console.log('');
      if (typeof optsGlobal.onQR === 'function') optsGlobal.onQR(qr);
    }
    ...
    if (connection === 'close') {
      ...
      if (!watchdogTimer) {
        watchdogTimer = setTimeout(() => {
          logger.error(`[WATCHDOG] Bot offline por mais de ${WATCHDOG_TIMEOUT / 1000}s. Reiniciando processo.`);
          process.exit(1);
        }, WATCHDOG_TIMEOUT);
      }
